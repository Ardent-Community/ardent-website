...
class SQLite3DatabaseHandler():
    ...
    def get_tables(self):
        with self.connect() as db:
            cursor = db.cursor()
            output = cursor.execute("SELECT name FROM sqlite_master WHERE type='table';").fetchall()
    ...
    def insert_values(self, challenge_num, uname, language, code):
        with self.connect() as db:
            ...
            cursor.execute(f"INSERT INTO solution{challenge_num} VALUES('{uname}', '{language}', '{code}')")
